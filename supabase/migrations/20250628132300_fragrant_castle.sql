/*
  # Fix deployment progress policies

  1. Changes
     - Add checks to prevent creating duplicate policies
     - Add visual progress tracking fields to android_deployments
     - Create deployment_progress_events table with proper policies
     - Create deployment_progress table with proper policies
...
-- Create policies for deployment_progress_events with existence checks
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deployment_progress_events' 
    AND policyname = 'Service role can manage progress events'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deployment_progress_events' 
    AND policyname = 'Users can view their own progress events'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deployment_progress_events' 
    AND policyname = 'Users can insert their own progress events'
  ) THEN
...
-- Create policies for deployment_progress with existence checks
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deployment_progress' 
    AND policyname = 'Service role can manage deployment progress'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deployment_progress' 
    AND policyname = 'Users can view their own deployment progress'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'deployment_progress' 
    AND policyname = 'Users can update their own deployment progress'
  ) THEN
...
-- Create function to update deployment progress based on events if it doesn't exist
...
-- Create trigger to update deployment progress if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
