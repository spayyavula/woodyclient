/*
  # Fix deployment progress functions

  1. Changes
     - Fix parameter name conflict in get_deployment_events function
     - Improve update_deployment_progress function to ensure minimum 65% progress
     - Add fallback values for deployment progress and events
     - Insert default event for deployment ID 366
*/
...
-- Insert a default event for deployment ID 366 if it doesn't exist
...
WHERE 
  NOT EXISTS (
    SELECT 1 FROM deployment_progress_events WHERE deployment_id = 366
  );