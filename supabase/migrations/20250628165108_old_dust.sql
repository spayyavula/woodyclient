/*
  ...
  1. New Tables
    - `deployment_progress_events` - Tracks individual events during deployment
      - `id` (bigint, primary key)
      - `deployment_id` (bigint, references android_deployments)
      - `event_type` (text, enum-like constraint)
      - `message` (text)
      - `percentage` (integer)
      - `timestamp` (timestamptz)
      - `metadata` (jsonb)
    
    - `deployment_progress` - Tracks step-by-step progress
      - `id` (bigint, primary key)
      - `deployment_id` (bigint, references android_deployments)
      - `step` (text)
      - `progress` (integer)
  ...
  2. Changes
    - Add progress tracking fields to android_deployments
    - Create function to update deployment progress
...
-- First drop any existing triggers that might depend on the function
DROP TRIGGER IF EXISTS update_deployment_progress_on_event ON deployment_progress_events;

-- Then drop the function if it exists with CASCADE to handle dependencies
